...
strat = {
    "name": "MATH-C6: Volume Profile Convergence",
    "sections": ["includePriceAction", "includeVolume"],
    "conditions": [],
    "rules": [
        {"signalId": "volume_obv_bull_divergence", "allocation": 3, "required": False},
        {"signalId": "volume_obv_bear_divergence", "allocation": 3, "required": False},
        {"signalId": "volume_surge", "allocation": 1, "required": False}
    ]
}
...
if strategy_id and "planToken" in data:
    ...
    agent_name = f"Fleet {strat['name'].split(':')[0]}"
...
for coin in coins:
    ...
    call_mcp("upsert_deployment_policy", radar_req)
    time.sleep(1)

print("\nDone! 25th Agent fully deployed.")
