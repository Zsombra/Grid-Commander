...
Generated from a live `initialize` (instructions), `tools/list`, `prompts/list` +
`prompts/get`, and `resources/list` + `resources/read` against
...
Returns: `accuracyPercent`, `agentAvatarUrl`, `agentConfidence`, `agentConfidencePercent`, `agentDisplayName`, `agentId`, `agentModelImageUrl`, `agentModelName`, `agentReasoning`, `avatarUrl`, `captainRank`, `cellScores`, `correctCount`, `grid`, `jackpotPattern`, `netChangeCapture`, `sessionId`, `submittedAt`, `totalChangeCapture`, `totalCount`, `userId`, `username`
...
Returns: `agentSlots`, `balance`, `mcpWagerEnabled`, `stats`, `tradingWalletProvisioned`, `username`
...
Returns: `currentUser`, `filter`, `generatedAt`, `leaderboard`
...
Returns: `coins`, `gridSize`, `presetDisplayName`, `primaryTimeframe`, `sections`, `sessionId`
...
Returns: `coins`, `strategyTimeframe`
...
Returns: `aggregateScorePercent`, `allEvaluatedSignals`, `coinImageUrl`, `coinName`, `coinTicker`, `comparison`, `currentPrice`, `dominantBias`, `hasConflictingSignals`, `isAgentWeighted`, `priceChangePercent`
...
Returns: `agentDisplayName`, `confidence`, `confidenceThreshold`, `gridPick`, `isUpdate`, `meetsThreshold`, `reasoning`
...
Returns: `assignablePresets`, `assignments`, `username`
...
Returns: `recentActivity`, `recentGames`, `recentThoughts`, `username`
...
Returns: `entries`, `limit`, `page`, `total`
...
Returns: `events`, `limit`, `page`, `total`
...
Returns: `aggregations`, `currentUser`, `entries`, `filter`, `generatedAt`, `stats`
...
Returns: `coinCount`, `primaryTimeframe`, `sessionId`, `snapshotTimestamp`, `sources`
...
Returns: `coinCount`, `content`, `displayName`, `estimatedTokenCount`, `snapshotTimestamp`, `sourceKey`, `tokenCountModel`
...
Returns: `budgets`, `category`, `metrics`, `previewExecutionLimits`, `templates`, `timeframeRefs`, `timeframes`, `transforms`
...
Returns: `budgetUsage`, `conditionColumns`, `conditionOutcomes`, `conditionVerdictTally`, `rankScopingNote`, `renderedSections`, `tokenCountModel`
...
Returns: `approvedPlan`, `planToken`, `reviewContext`
...
Returns: `appliedImpact`, `strategy`
...
Returns: `avgAggregateScore`, `avgAggregateScorePercent`, `avgConviction`, `avgConvictionPercent`, `avgDurationSeconds`, `avgNetPnl`, `avgRiskRewardRatio`, `blockedCount`, `cancelledCount`, `enterCount`, `executedCount`, `expiredCount`, `failedCount`, `fillRatePct`, `lossCount`, `outcomeCount`, `pendingCount`, `skipCount`, `skippedCount`, `topCoinsByEvaluation`, `totalEntryDecisions`, `totalEvaluations`, `totalNetPnl`, `winCount`, `winRate`, `winRatePercent`
...
Returns: `aggregateScore`, `aggregateScorePercent`, `attributions`, `gate`, `gatePercent`, `wouldRoute`
...
Returns: `activeCoinTickers`, `agents`, `positions`, `totals`, `userId`
...
Returns: `activeCoinTickers`, `agents`, `marketGridSessionId`, `positions`, `totals`, `userId`
...
Returns: `closeOrderAccepted`, `decisionId`, `outcomeId`
...
Returns: `authoringContext`, `policy`
...
Returns: `presetId`, `revision`, `slotCount`
...
Returns: `blockedAgents`, `policies`, `summary`
...
Returns: `events`, `nextCursor`, `samples`, `samplesAgentId`, `samplesAgentName`
...
Returns: `snapshot`, `symbol`, `timeframe`
...
Returns: `points`, `symbol`, `timeframe`
...
Returns: `limit`, `page`, `total`, `trades`
...
Bodies are the server's own text, verbatim; the server tells clients not to
copy catalogs, formulas, or defaults out of them — they describe *sequence*,
and the values are rediscovered live.
...
What every client is told at `initialize`, verbatim. This is the platform's
prose contract — scope semantics, workflow sequences, per-tool pagination and
rate limits live here and nowhere in the schemas. The freshness guard compares
this text (addressee-normalised) against the live server.
...
- `list_gate_blocks({ agentId, page?, limit? })`
  Paginated pre-signal pipeline rejections — why candidate signals were gate-blocked. Use to debug an agent that isn't trading.
- `get_decision_order_attribution()`
