...
- [ ] 1.1 Domain types in `src/domain/recording/capture.ts`: SignalCapture
...
- [ ] 1.2 `readCoinSignalPreview` on the strategies port surface, typed to
      the observed v11.0.0 shape → "A Capture Records What Every Signal Said"
- [ ] 1.3 `SignalRecordStore` port in `src/ports/signal-record.ts`: append
...
- [ ] 2.1 Drizzle tables `signal_captures` (with raw payload column and
...
- [ ] 2.2 `signal-record-repo.ts` implementing the store port
- [ ] 2.3 DB tests (`tests/db/signal-record.test.ts`): round-trip a capture
...
- [ ] 3.1 `signal-preview-mapper.ts` from the observed shape — keep every
...
- [ ] 3.2 Adapter method on the BattleGrid MCP adapter; envelope unwrapped;
...
- [ ] 3.3 Conformance fixture from `docs/battlegrid-mcp-surface.json`'s
      observed shape so the payload-conformance suite covers the new read

## 4. The capture command

- [ ] 4.1 `capture-signals.command.ts`: coin-set resolution (named coins +
...
- [ ] 4.2 Per-coin capture loop: one coin's failure recorded as a gap with
...
- [ ] 4.3 Stamp capture time (injected clock, not a DB default) and the
      platform server version on every capture
      → "A Capture Records What Every Signal Said"
- [ ] 4.4 Unit tests over fakes: full capture, partial failure, total
      failure, empty coin set, provenance both ways

## 5. Unattended entry

- [ ] 5.1 `bin/grid-commander-record.ts` on the `grid-commander-mcp.ts`
...
- [ ] 5.2 Exit semantics: zero only when at least one coin recorded; summary
      ...
      "Nothing to cover"
- [ ] 5.3 No mutating tool named, no `*Command` against BattleGrid — the
      live-writes architecture gate stays green by construction

## 6. Read surfaces (web)

- [ ] 6.1 Coverage page: recording started / last capture / count / gaps per
...
- [ ] 6.2 Per-coin timeline: captures in time order with aggregate, bias,
      conflict, price; every reading carries its capture time
      → "Recorded History Is Readable By Coin And By Signal"
- [ ] 6.3 Per-signal history view for one coin → same requirement
- [ ] 6.4 Provenance rendered with history (named vs deployments-at-the-time)
      → "The Coins Captured And Why Are Recorded"
- [ ] 6.5 Rendering tests: gap visible, never-recorded vs unreadable,
      capture-time on every reading, cross-account isolation via fakes

## 7. The MCP boundary

- [ ] 7.1 Two read tools in `src/mcp/tools.ts` calling the same queries:
...
- [ ] 7.2 Read-only reachability guard: the tools reach no BattleGrid write
      and no capture-performing use-case (capture stays operator-invoked)
- [ ] 7.3 MCP tests: gap crosses as a gap; never-recorded says where
      recording starts; unreadable store crosses as data naming itself

## 8. Live proof

- [ ] 8.1 Key-gated `tests/live/recorder-probe.test.ts`: one real
...
- [ ] 9.1 `docs/BATTLEGRID_SURFACE_MAP.md`: `get_coin_signal_preview` moves
      to the consumed table (52 → 53)
- [ ] 9.2 `docs/MCP_SERVER.md`: the two tools, and that the record is this
      product's own store
- [ ] 9.3 Operator note (FIRST_SESSION or README pointer): starting the
      recorder — the one-line cron and why gaps are permanent

## 10. Verification

- [ ] 10.1 `./scripts/ci.sh` green keyless (typecheck, lint, tests, build,
      db:generate diff clean, harness)
- [ ] 10.2 Scenario walk: every scenario in both delta specs traced to a
      passing test or a recorded live observation, listed in the change
      folder
- [ ] 10.3 `python3 .claude/tools/openspec.py validate
