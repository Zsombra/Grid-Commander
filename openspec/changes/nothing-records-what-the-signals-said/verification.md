...
| No credential | **Weaker than a test, stated plainly**: the refusal branch in `bin/grid-commander-record.ts` is the same `currentUser.kind !== 'acting'` check `bin/grid-commander-mcp.ts` ships (itself proven by the live `mcp-server-probe`), and it cannot be unit-driven without a composition boot that requires real config. The exit-code mapping around it is pure and tested (`cli-exit.test.ts`); nothing writes to the store before `captureSignals.execute` is reached. |
