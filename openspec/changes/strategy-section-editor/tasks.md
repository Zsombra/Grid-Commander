...
- [x] 1.1 Add `SectionTemplate` discriminated union to `src/domain/strategy/strategy.ts`:
...
- [x] 2.1 Add `VocabularyTemplatesResult` union type to `src/ports/strategies.ts`:
      `'templates'` (carries `templates: SectionTemplate[]`) | `'unreadable'`
- [x] 2.2 Add `CategoryOptions` type: `{ category: string; label: string; templates: SectionTemplate[] }`
- [x] 2.3 Add `SectionOptionsResult` union: `'ready'` (detail + categories) |
      `'strategy-unreadable'` | `'strategy-missing'` | `'vocabulary-unreadable'`
- [x] 2.4 Add `listVocabularyTemplates(params: { authority }):
...
- [x] 3.1 Add `vocabulary` key to `TOOLS` constant in `strategy-adapter.ts`
      mapping to `'list_strategy_vocabulary'`
- [x] 3.2 Implement `listVocabularyTemplates` in `McpStrategyAdapter`: call the
      ...
      `ToolRefusedError`
- [x] 3.3 Map defensively to discriminated union: if entry has `sectionKey` →
      `{ kind: 'platform', sectionKey, label }`; if entry has `templateKey` →
      `{ kind: 'custom', templateKey, label }`; skip entries with neither
- [x] 3.4 Update `mapCategory` in the adapter to carry optional guidance fields:
      `whenToUse`, `bestPractices`, `commonMisuses`, `examples` — added to
      `VocabularyCategory` in port (guidance fields are port-level, not domain-level)

## 4. Use case — ReadSectionOptionsQuery
- [x] 4.1 Create `src/application/use-cases/read-section-options.query.ts`
- [x] 4.2 Constructor receives `StrategiesPort`
- [x] 4.3 `execute({ authority, strategyId })`:
...
- [x] 6.1 Replace `listStrategies`+`readVocabulary` calls with `readSectionOptions`
- [x] 6.2 Handle `'strategy-missing'`, `'strategy-unreadable'`, and
      `'vocabulary-unreadable'` result kinds with appropriate error states
- [x] 6.3 When `result.kind === 'ready'` and no `compile` param: render the
      compose form with:
...
- [x] 6.4 When `compile=1` param present: reconstruct sections from `sections[]`
...
- [x] 6.5 `intentSummary` and `assumptions` reflect what actually changed
      (tagline only / sections only / both)
- [x] 6.6 Ensure the "back to strategy" link is present on every state this
...
- [x] 7.1 **Test**: `ReadSectionOptionsQuery` with a strategy that has sections
...
- [x] 7.2 **Test**: `ReadSectionOptionsQuery` when `readVocabulary` returns
      `'unreadable'` → result is `'vocabulary-unreadable'` (traces to:
      vocabulary unavailable blocks section editing)
- [x] 7.3 **Test**: `ReadSectionOptionsQuery` when `readStrategy` returns
      `'missing'` → result is `'strategy-missing'` (traces to: compose form
      guarded by strategy reachability)
- [x] 7.4 **Test**: adapter `listVocabularyTemplates` maps platform templates
...
- [x] 7.5 **Test**: adapter `listVocabularyTemplates` skips entries missing both
      `sectionKey` and `templateKey` rather than throwing (traces to: defensive
      mapping)
- [x] 7.6 **Test**: compile intent from edit page carries `sections` array
...
- [x] 7.7 Mutate: remove section from compile intent → test 7.6 fails
- [x] 7.8 Mutate: inject a hard-coded section key instead of reading from
      vocabulary → test 7.4 fails (traces to: sections not invented)
- [x] 7.9 Run `scripts/check.sh` — all gates green

## 8. Out-of-scope backlog items
- [x] 8.1 File `strategy-metric-editor` backlog item for metric construction
      hints and column contracts (get_metric_construction_hints,
      get_strategy_column_contract)
- [x] 8.2 File `strategy-draft-preview` backlog item for preview_strategy_report
- [x] 8.3 Update `strategy-section-editor` backlog item:
