...
- [ ] 1.1 Add `SectionTemplate` to `src/domain/strategy/strategy.ts`:
      `{ kind: string; sectionKey: string; label: string; description: string }`
      — the stable interface; the adapter maps the raw vocabulary payload to this

## 2. Port — listCategoryVocabulary
- [ ] 2.1 Add `CategoryVocabularyResult` union type to `src/ports/strategies.ts`:
      `'vocabulary'` (carries `templates: SectionTemplate[]`) | `'unreadable'`
...
- [ ] 2.4 Add `listCategoryVocabulary(params: { authority; category: string }):
      Promise<CategoryVocabularyResult>` to `StrategiesPort` interface
...
- [ ] 3.2 Implement `listCategoryVocabulary` in `McpStrategyAdapter`: call the
      tool with `{ category }`, map response `templates` array to
      `SectionTemplate[]`; return `'unreadable'` on `ToolRefusedError`
- [ ] 3.3 Map defensively: extract `kind`, `sectionKey`/`templateKey`, `label`,
      `description` from each template object; skip entries missing required fields
...
- [ ] 4.3 `execute({ authority, strategyId })`:
      1. Concurrently: `readStrategy` + `readVocabulary`
      2. Return `'strategy-missing'` / `'strategy-unreadable'` / `'vocabulary-unreadable'`
         as appropriate
      3. For each category, concurrently call `listCategoryVocabulary`
      4. Skip categories whose vocabulary returns `'unreadable'` (log, don't fail)
      5. Return `{ kind: 'ready', detail, categories: CategoryOptions[] }`
...
- [ ] 7.4 **Test**: adapter `listCategoryVocabulary` maps vocabulary templates
      correctly; `ToolRefusedError` → `'unreadable'` (traces to: sections from
      platform vocabulary, not invented)
- [ ] 7.5 **Test**: adapter `listCategoryVocabulary` skips malformed template
      entries (missing sectionKey) rather than throwing (traces to: defensive
      mapping)
