...
- [ ] 1.1 `app/(app)/agents/[id]/archive`: extract `performArchive` to
      `actions.ts` (`'use server'` top, imports carried), page imports
      `./actions.js`
- [ ] 1.2 Same for `agents/[id]/deploy` (`performDeploy`)
- [ ] 1.3 Same for `agents/[id]/edit` (`applyEdit`)
- [ ] 1.4 Same for `agents/[id]/reactivate` (`reactivate`)
- [ ] 1.5 Same for `agents/[id]/rebind` (`performRebind`)
- [ ] 1.6 Same for `agents/[id]/undeploy/[coin]` (`performUndeploy`)
- [ ] 1.7 Same for `agents/new` (`create`)
- [ ] 1.8 Same for `recorder/trim` (`performTrim`)
- [ ] 1.9 Same for `strategies/[id]/archive` (`archiveStrategy`)
- [ ] 1.10 Same for `strategies/[id]/conditions/save` (`saveConditions`)
- [ ] 1.11 Same for `strategies/[id]/edit` (`apply`)
- [ ] 1.12 Same for `strategies/[id]/fork` (`forkStrategy`)
- [ ] 1.13 Same for `strategies/[id]/restore` (`restoreStrategy`)
- [ ] 1.14 Same for `strategies/[id]/rules/[signalId]` (`performRetune`)
- [ ] 1.15 `agents/new/page.tsx`: drop the `= {}` parameter default so the
      props type stops widening to `| undefined` (PageProps scenario)

## 2. The gate

- [ ] 2.1 `tsconfig.json`: `exclude` stops swallowing `.next/types`
      (drop `.next`; fall back to narrow enumeration only if 2.2 shows sweep)
- [ ] 2.2 After a real `npm run build` into `.next`, confirm via
      `tsc --listFiles` (or `--explainFiles`) that the only `.next` files in
      the program are under `.next/types`
- [ ] 2.3 Confirm `next build` no longer rewrites `tsconfig.json` (or that
      what it writes back is what we committed) — the item records the
      rewrite as the trap for any exclude-based fix
- [ ] 2.4 Prove the gate: re-add one forbidden export temporarily, watch
      `npm run build` fail naming the route, revert (contract-violation
      scenario)
- [ ] 2.5 Revert the investigation artifacts: `next.config.ts` distDir
      wiring out, `.next-gate/` deleted, Next's `.next-gate` include entry
      out of `tsconfig.json`

## 3. Test ecosystem follows the move

- [ ] 3.1 Re-point named-action imports:
...
- [ ] 3.2 `tests/rendering/new-agent.test.ts`: three bare `Page()` calls
      become `Page({})`
- [ ] 3.3 `a-form-sends-what-its-action-reads.test.ts`: resolve the form for
...
- [ ] 3.4 Re-point source-slicing tests the move breaks:
      ...
      same way
- [ ] 3.5 Vacuity sweep of the re-points: for each test re-pointed in 3.4,
...
- [ ] 4.1 New `tests/architecture/tsconfig-coherence.test.ts`: parse
...
- [ ] 4.2 Prove it per the guard-proof requirement: the proof drives the
...
- [ ] 5.1 Re-pin every surface manifest staled by the page edits, adding the
