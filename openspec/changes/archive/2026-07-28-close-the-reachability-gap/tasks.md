...
- [x] 1.1 `tests/architecture/reachability.test.ts` — half one: extract every
      path the presentation layer can render and resolve each against the route
      tree, expanding dynamic segments
- [x] 1.2 Half two: no `<form>` carries a string or template `action`, and every
...
- [x] 2.1 `agent-form.tsx` — require an `action` prop; drop the hardcoded
      `method`/`action`
- [x] 2.2 `rebind-confirm.tsx` — same
- [x] 2.3 `plan-review.tsx` — same
- [x] 2.4 `agents/new/page.tsx` — pass `create`
- [x] 2.5 `agents/[id]/rebind/page.tsx` — pass `performRebind`
- [x] 2.6 `agents/[id]/page.tsx` — render a rename form and pass `rename`; it is
      currently an action with no form at all
- [x] 2.7 `strategies/[id]/edit/page.tsx` — write the apply action that does not
      exist, and pass it to `PlanReview`

## 3. The five missing routes

- [x] 3.1 `agents/[id]/edit` — the agent-owned fields, using `UpdateAgentCommand`.
      Render `rejected` and `invalid` results as named field errors, not a
      generic failure
- [x] 3.2 `agents/[id]/reactivate` — its own consequence copy, not archive's
- [x] 3.3 `strategies/[id]/fork` — no confirmation token; forking changes nothing
      that exists
- [x] 3.4 `strategies/[id]/archive` — confirmation token carrying the blast
      radius, following the agent archive page
- [x] 3.5 `strategies/[id]/restore` — handle `repair-required` as a state with a
      way forward, using `REPAIR_REQUIRED_GUIDANCE`

## 4. Verification

- [x] 4.1 The guard passes — R: Every Affordance The Interface Offers Resolves,
      R: Every Form The Interface Renders Can Be Submitted
- [x] 4.2 Re-inject each defect class and confirm the guard fails: one link to a
...
- [x] 4.3 Serve the built application and request all thirteen-plus routes; none
      returns 404 — R: Every Capability Is Reachable, scenarios Authoring agents
      and Authoring strategies
- [x] 4.4 Submit each of the six write paths against the served application and
...
- [x] 4.5 Confirm the honest ceiling in writing: no BattleGrid round trip was
      proven, and why — R: the proposal's stated limit
- [x] 4.6 `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`,
