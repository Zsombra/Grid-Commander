...
- [ ] 1.1 Add a local `$ref` resolver to `tools/probe_mcp_surface.py`:
...
- [ ] 1.2 Add `input_required_paths(tool)`: every required field as a dotted
      ...
      instead.
- [ ] 1.3 Add `input_accepts(tool)`: per object path with
...
- [ ] 1.4 Add `--refresh-declared`: load `docs/battlegrid-mcp-capabilities.json`,
      ...
      no key.
- [ ] 1.5 Run the refresh and commit the regenerated artifact; confirm in the
      diff that only declared fields changed.

## 2. The guard — tests

- [ ] 2.1 `tests/test_probe_declared_fields.py` (stdlib-only): pin the
      ...
      observed fields.
- [ ] 2.2 `tests/architecture/payload-conformance.test.ts`: build the create
...
- [ ] 3.1 `pnpm typecheck`, `pnpm lint`, `pnpm test` — all green, new guard
      included.
- [ ] 3.2 The Python harness suite passes on a clean checkout with no packages
      installed (`./scripts/check.sh` gates).
- [ ] 3.3 `python3 .claude/tools/openspec.py validate
