...
export interface PositionManagementPreset {
   ...
   * it — the full set of behavioural values (twelve at v17.2.0). `null` when
   * the catalog did not supply one, and a preset without its configuration is
   * not offerable: the label is sent alongside the values, never instead of
   * them, and this product does not invent the values a complete payload
   * needs.
   */
