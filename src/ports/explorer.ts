 ...
 *
 * The domain scorecard without `cost`: BattleGrid nulls the owner-only
 * telemetry on every public read, so a competitor's evaluation can never
 * carry what it cost to think.
 */
export type EvaluationDetail = Omit<EvaluationScorecard, 'cost'>;
