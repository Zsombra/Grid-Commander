...
| | | |
|---|---|---|
| **#145** | Is the signal-recorder cron running? A gap can never be backfilled | **First.** Five-minute operator deployment check; its answer unlocks `/propose` on #94, the largest open build |
...
- **Waiting on the operator**: `confirm-the-recorder-is-running` (#145, the
  five-minute check), `prove-token-lifetimes` and
