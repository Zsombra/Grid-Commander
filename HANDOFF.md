...
**Date**: 2026-08-01  
**State**: green (1016 vitest + 62 db + 221 harness tests, all nine `./scripts/ci.sh` gates; 17 further vitest are key-gated live probes). No active changes. 22 open backlog items. PRs #8–#28 merged. The assistant roadmap (`an-assistant-over-the-use-cases`) is filed; **Phase 1 (strategy-maker) is complete** — signal vocabulary, metric/column workbench, the signal-rule write (live-proven), and the agent's-eye preview. Phase 2 is reporting/EV (`trading-telemetry-is-unread`, `entry-decisions-have-a-read-side`).
...
| Changes (archived) | 72 |
| Vitest tests | 1016 (+17 key-gated live) + 62 db |
| Harness tests (Python) | 221 |
| Active changes | 0 |
| Open backlog items | 22 |
| Design tickets open | 0 |
| Open draft PRs | 0 (see PR list; #8–#28 merged) |
